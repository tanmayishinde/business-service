...
public class ProductChildPartServiceImpl implements ProductChildPartService {
    ...
    @Autowired
    public ProductChildPartServiceImpl(DataRetrieveProxy dataRetrieveProxy,
    ...
    public DataRetrieveBusinessVO getDataFromDb() {
        DataRetrieveBusinessVO dataRetrieveBusinessVO = new DataRetrieveBusinessVO();

        String sessionId = sessionUtil.getSessionId();
        try {
            SupplierContext supplierContext = new SupplierContext();
            List<ProductDisplayVO> productDisplayVOList = dataRetrieveProxy.getProductList();
            List<ChildPartDisplayVO> childPartList = dataRetrieveProxy.getChildPartList();
            Map<Long, List<ChildPartQuantityVO>> productChildPartList = dataRetrieveProxy.getProductChildPartList();
            List<ProductChildPartVO> productChildParRelationVOList = dataRetrieveProxy.getproductChildPartRelationshipList();
            supplierContext.setProductList(productDisplayVOList);
            supplierContext.setChildPartList(childPartList);
            supplierContext.setProductChildPartList(productChildPartList);
            supplierContext.setProductChildPartRelationshipVOList(productChildParRelationVOList);
            redisContextWrapper.setContext(sessionId, supplierContext);
            dataRetrieveBusinessVO.setProductDisplayVOList(dataRetrieveProxy.getProductList());
            dataRetrieveBusinessVO.setChildPartList(dataRetrieveProxy.getChildPartList());
            dataRetrieveBusinessVO.setProductChildPartList(dataRetrieveProxy.getProductChildPartList());
            dataRetrieveBusinessVO.setProductChildPartVOList(dataRetrieveProxy.getproductChildPartRelationshipList());
        } catch (ValidationException e) {
            e.printStackTrace();
        }
        return dataRetrieveBusinessVO;
    ...
    public List<MonthlyDisplayVO> getMonthlyPlanVODisplayList(
        ...
        for (MonthlyPlanEntityVO monthlyPlanEntity : monthlyPlanEntityVOList) {
            ...
            List<ProductChildPartVO> productChildPartVOList = findProductRelatedProductChildpartList(product.getProductID(), supplierContext);
            for (ProductChildPartVO getChildPartId : productChildPartVOList) {
    ...
    public List<ChildPartWeeklyPlan> getChildPartWeeklyPlan(List<MonthlyPlanEntityVO> monthlyPlanEntityVOList, SupplierContext supplierContext) {
        ...
        for (ChildPartDisplayVO childPartEntity : childPartEntityList) {
            ChildPartWeeklyPlan childPartWeeklyPlan = new ChildPartWeeklyPlan();
            List<ProductChildPartVO> productChildPartVOList = findChildPRelatedProductChildpartList(childPartEntity.getChildPartID(), supplierContext);
            ...
            for (ProductChildPartVO productChildPartEntity : productChildPartVOList) {
    ...
    public List<ProductChildPartVO> findChildPRelatedProductChildpartList(Long childPartEntity, SupplierContext supplierContext) {
        List<ProductChildPartVO> childPartRelatedProductChildpartList = new ArrayList<>();

        for (ProductChildPartVO productChildPartVO : supplierContext.getProductChildPartRelationshipVOList()) {
    ...
    public List<ProductChildPartVO> findProductRelatedProductChildpartList(Long productID, SupplierContext supplierContext) {
        List<ProductChildPartVO> findProductRelatedProductChildpartList = new ArrayList<>();
